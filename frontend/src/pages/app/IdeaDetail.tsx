      ...
      {idea.status === 'completed' && analyses.length > 0 && (
        <div className="space-y-8">
          ...
          {getAnalysis('business_values') && (
            <AnalysisSection title="Business Values" description="Core differentiators and strategy">
              <div className="space-y-6">
                <div>
                  <h4 className="mb-3 font-semibold">Product Differentiators (Moats)</h4>
                  <ul className="space-y-2">
                    {getAnalysis('business_values')!.content.moats.map((moat: string, idx: number) => (
                      <li key={idx} className="text-sm text-text-secondary">• {moat}</li>
                    ))}
                  </ul>
                </div>
                <div>
                  <h4 className="mb-3 font-semibold">Target Market</h4>
                ...
                <div>
                  <h4 className="mb-3 font-semibold">Pricing Strategies</h4>
                  {getAnalysis('business_values')!.content.pricingStrategies.map((strategy: any, idx: number) => (
                    <div key={idx} className="mb-3">
                      ...
                      <p className="text-sm text-text-secondary">{strategy.rationale}</p>
                    </div>
                  ))}
                </div>
                <div>
                  <h4 className="mb-3 font-semibold">Timeline to Market</h4>
          ...
          {getAnalysis('pmf') && (
            <AnalysisSection title="Product-Market Fit Strategies" description="Quick validation approaches">
              <div className="space-y-4">
                {getAnalysis('pmf')!.content.strategies.map((strategy: any, idx: number) => (
                  <div key={idx} className="rounded-lg border border-border p-4">
                    ...
                    <p className="mt-3 text-sm text-text-secondary">{strategy.description}</p>
                  </div>
                ))}
              </div>
            </AnalysisSection>
          ...
          {getAnalysis('next_steps') && (
            <AnalysisSection title="Next Steps" description="Recommended actions to get started">
              <div className="space-y-4">
